...
class ProductController extends Controller
{
    ...
    /**
     * Stream the QR code as a PNG (converted server-side from SVG via Imagick/GD fallback).
     * Falls back to serving the SVG if no image extension is available.
     */
    public function downloadQrPng(Product $product)
    {
        if (!$product->qr_code || !Storage::disk('public')->exists($product->qr_code)) {
            return back()->with('error', 'QR code not found.');
        }

        $svgContent = Storage::disk('public')->get($product->qr_code);
        $filename   = "qr-{$product->sku}.png";

        // Try Imagick first (most reliable SVG→PNG)
        ...
        // Fallback: serve SVG with a .png name hint (browser will still open it)
        return Storage::disk('public')->download($product->qr_code, $filename, [
