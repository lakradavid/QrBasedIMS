...
class ProductController extends Controller
{
    ...
    public function downloadQr(Product $product, string $format = 'png')
    {
        $format = in_array($format, ['svg', 'png']) ? $format : 'png';

        if ($format === 'svg') {
            $path = "qrcodes/qr-{$product->sku}.svg";
            if (!Storage::disk('public')->exists($path)) {
                // Regenerate if missing
                $this->qrCodeService->generate($product);
            }
            return Storage::disk('public')->download($path, "qr-{$product->sku}.svg");
        }

        // PNG
        $pngPath = "qrcodes/qr-{$product->sku}.png";
        if (!Storage::disk('public')->exists($pngPath)) {
            // Regenerate both SVG + PNG
            $qrPath = $this->qrCodeService->generate($product);
            $product->update(['qr_code' => $qrPath]);
        }

        if (Storage::disk('public')->exists($pngPath)) {
            return Storage::disk('public')->download($pngPath, "qr-{$product->sku}.png");
        }
