...
<div class="py-4 space-y-4" x-data="{ selected: [], selectAll: false }">
    ...
    <div x-show="selected.length > 0" x-cloak class="bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-3 flex items-center gap-4">
        ...
        <button @click="bulkDownloadQr(selected)"
                class="text-sm border border-indigo-300 text-indigo-700 px-3 py-1.5 rounded-lg hover:bg-indigo-100 transition-colors flex items-center gap-1.5">
                            ...
                            @if($product->qr_code)
                                <div class="flex items-center justify-center gap-1.5">
                                    <a href="{{ route('products.show', $product) }}" title="View QR">
                                        <img src="{{ $product->qr_code_url }}" class="w-8 h-8" alt="QR">
                                    </a>
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" x-transition
                                             class="absolute z-20 right-0 mt-1 w-40 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden text-left">
                                            <a href="{{ route('products.download-qr', $product) }}"
                                               class="flex items-center gap-2 px-3 py-2 text-xs text-gray-700 hover:bg-indigo-50 hover:text-indigo-700 transition-colors">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                                SVG
                                            </a>
                                            <a href="{{ route('products.download-qr-png', $product) }}"
                                               class="flex items-center gap-2 px-3 py-2 text-xs text-gray-700 hover:bg-indigo-50 hover:text-indigo-700 transition-colors border-t border-gray-100">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                                PNG
                                            </a>
                                            <button onclick="downloadQrPngClient('{{ $product->qr_code_url }}', '{{ $product->sku }}')"
                                                    class="flex items-center gap-2 w-full px-3 py-2 text-xs text-gray-700 hover:bg-indigo-50 hover:text-indigo-700 transition-colors border-t border-gray-100">
...
@push('scripts')
<script>
// Client-side SVG → PNG download (single product, used from table row dropdown)
function downloadQrPngClient(svgUrl, sku) {
    fetch(svgUrl)
        .then(r => r.text())
        .then(svgText => {
            const size = 1200;
            ...
            img.onerror = function () {
                URL.revokeObjectURL(url);
                alert('Could not convert QR to PNG. Try the server-side PNG download from the product page.');
            };
            img.src = url;
        })
        .catch(() => alert('Could not fetch QR image. Try the server-side PNG download from the product page.'));
}

// Bulk SVG download — opens each selected product's SVG download in a new tab
// (browsers will trigger individual file downloads)
function bulkDownloadQr(ids) {
    if (!ids || !ids.length) return;
    // Map of product id → download URL injected from Blade
    const downloadUrls = @json($products->pluck('id')->mapWithKeys(fn($id) => [$id => route('products.download-qr', $id)]));
    ids.forEach((id, i) => {
        const url = downloadUrls[id];
        if (!url) return;
        // Stagger slightly so browsers don't block multiple simultaneous downloads
        setTimeout(() => {
