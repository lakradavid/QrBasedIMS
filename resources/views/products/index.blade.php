                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                                    <button @click="open = !open" class="text-gray-400 hover:text-indigo-600 transition-colors" title="Download QR">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                    </button>
                                    <div x-show="open" x-cloak x-transition
                                         class="absolute right-0 mt-1 w-44 bg-white border border-gray-200 rounded-xl shadow-lg z-50 py-1 text-sm">
                                        {{-- SVG --}}
                                        <a href="{{ route('products.download-qr', ['product' => $product, 'format' => 'svg']) }}"
                                           class="flex items-center gap-2.5 px-4 py-2.5 text-gray-700 hover:bg-gray-50 transition-colors border-b border-gray-100">
                                        ...
                                        {{-- PNG (server) --}}
                                        <a href="{{ route('products.download-qr', ['product' => $product, 'format' => 'png']) }}"
                                           class="flex items-center gap-2.5 px-4 py-2.5 text-gray-700 hover:bg-gray-50 transition-colors border-b border-gray-100">
                                            <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                            PNG
                                        </a>
                                        {{-- PNG (browser canvas) --}}
                                        <button onclick="downloadQrBrowser('{{ $product->qr_code_url }}', '{{ $product->sku }}')"
                                                class="w-full flex items-center gap-2.5 px-4 py-2.5 text-gray-700 hover:bg-gray-50 transition-colors">
...
<script>
/**
 * PNG (browser) — loads the SVG into a canvas and triggers a PNG download
 * without any server round-trip.
 */
function downloadQrBrowser(svgUrl, sku) {
    const img = new Image();
    ...
    img.onload = function () {
        ...
        link.download = 'qr-' + sku + '-browser.png';
        link.href     = canvas.toDataURL('image/png');
        link.click();
    };
    img.onerror = function () {
        alert('Could not render QR image. Try the PNG download instead.');
    };
    img.src = svgUrl;
}
</script>
