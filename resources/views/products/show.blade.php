                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" @click.outside="open = false"
                                ...
                                Download QR
                                <svg class="w-3.5 h-3.5 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                            </button>
                            <div x-show="open" x-transition
                                ...
                                <a href="{{ route('products.download-qr-png', $product) }}"
                                   class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-700 transition-colors border-t border-gray-100">
                                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                    Download PNG
                                    <span class="ml-auto text-xs text-gray-400">400×400</span>
                                </a>
                                {{-- PNG (client-side canvas) --}}
                                <button onclick="downloadQrPngClient('{{ $product->qr_code_url }}', '{{ $product->sku }}')"
                                        class="flex items-center gap-2.5 w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-700 transition-colors border-t border-gray-100">
                                    ...
                                    Save as PNG (browser)
                                    <span class="ml-auto text-xs text-gray-400">Client</span>
                                </button>
                            </div>
                        </div>
...
function downloadQrPngClient(svgUrl, sku) {
    // Fetch the SVG text so we can set an explicit width/height before rasterising.
    // This forces the browser to render it at high resolution instead of the tiny
    // intrinsic size, giving crisp QR modules that scanners can read.
    fetch(svgUrl)
        .then(r => r.text())
        .then(svgText => {
            const size = 1200; // high-res so QR modules are sharp

            // Inject explicit dimensions into the SVG root element
            ...
            img.onerror = function () {
                URL.revokeObjectURL(url);
                alert('Could not convert QR to PNG in browser. Try the server-side PNG download instead.');
            };
            img.src = url;
        })
        .catch(() => {
            alert('Could not fetch QR image. Try the server-side PNG download instead.');
        });
}
</script>
@endpush
